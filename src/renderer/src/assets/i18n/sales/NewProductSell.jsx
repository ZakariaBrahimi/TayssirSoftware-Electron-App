...
const NewProductSell = () => {
  ...
  const getTotalNetProfit = () => {
    ...
    soldProducts.forEach((product) => {
      total +=
        (product?.product?.price - product?.product?.cost - product?.discount) * product?.quantity
    })
    return total
  }
  useEffect(() => {
  ...
  const SearchByBarcodeRef = useRef(null)
  const SearchByBarcode = (barcode) => {
    // sending the barcode value to the server and receiving the response
    // if the product not available ===>> push a notification
    // if the product is available  ===>> add it to the sold Product list
    //                              ===>> empty search input field

    window.electron.ipcRenderer.send('SearchByBarcode', barcode)
    window.electron.ipcRenderer.once('SearchByBarcode:response', (event, response) => {
      if (response.success) {
        console.log('hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh', response)
        const product = response.product.dataValues
        if (!soldProducts.find((soldProduct) => soldProduct.product.id === product.id)) {
          setSoldProducts((prev) => [...prev, { product: product, quantity: 1, discount: 0 }])
        } else {
          setSoldProducts((prev) =>
        ...
        SearchByBarcodeRef.current.value = ''
      } else {
        console.error('The product is not avalaibl : ', response.message)

        toast({
          description: `The Product with barcode - ${barcode} - doesn't Exist`,
          variant: 'destructive'
        })
      }
    })
      ...
      <DialogContent className="sm:max-w-[925px]">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            <Tabs defaultValue="Barcode" className="col-span-4">
              ...
              <TabsContent className="mt-0 mb-6" value="Barcode">
                ...
                <Input
                  className="mt-8"
                  ref={SearchByBarcodeRef}
                  onChange={(e) => SearchByBarcode(e.target.value)}
                  type="number"
                          ...
                          >
                            ...
                            <TableCell className="text-center">
                              <Input
                                ...
                                onChange={(event) =>
                                  handleProductDiscountChange(soldProduct, event)
                                }
                              />
                            </TableCell>
                            <TableCell className="text-center">
                              <Input
                                ...
                                onChange={(event) =>
                                  handleProductQuantityChange(soldProduct, event)
                                }
                              />
                            </TableCell>
        ...
        <DialogFooter>
          <div className="flex justify-between w-full">
            ...
            <Button onClick={() => newSoldProducts(soldProducts, setDialogIsOpen)} type="button">
              Sold
            </Button>
          </div>
        </DialogFooter>
